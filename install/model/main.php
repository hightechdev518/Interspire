...
class ModelMain extends Model {
	...
	public function checkRequirements(){
		...
		if (!extension_loaded('gd')) {
            $errors[] = 'Warning: GD extension needs to be loaded! In php.ini set extension=gd, then restart Apache (XAMPP Control Panel) or stop and restart the PHP built-in server — PHP only reads php.ini on startup. Open install/check_env.php in your browser to verify.';
		}
		...
		if (!function_exists('mcrypt_encrypt') && !function_exists('openssl_encrypt')) {
            $errors[] = 'Warning: OpenSSL extension needs to be loaded (or legacy mCrypt)!';
		}
